...
mod tests {
    use super::*;
    use crate::exobyte_format::{write_f64_le, write_i32_le, write_u16_le, write_u32_le};

    fn encode_function(name: &str, strings: &[&str], instr: &[u8]) -> Vec<u8> {
        let mut code = Vec::new();
        write_u16_le(&mut code, strings.len() as u16);
        for s in strings {
            write_u16_le(&mut code, s.len() as u16);
            code.extend_from_slice(s.as_bytes());
        }
        code.extend_from_slice(instr);

        let mut out = Vec::new();
        write_u16_le(&mut out, name.len() as u16);
        out.extend_from_slice(name.as_bytes());
        write_u32_le(&mut out, code.len() as u32);
        out.extend_from_slice(&code);
        out
    }

    fn build_program(functions: Vec<Vec<u8>>) -> Vec<u8> {
    ...
    fn vm_runs_empty_main() {
        let instr = vec![Opcode::Ret.byte(), 0];
        let bytes = build_program(vec![encode_function("main", &[], &instr)]);
    ...
    fn vm_runs_bool_ops() {
        ...
        instr.push(Opcode::LoadBool.byte());
        write_u16_le(&mut instr, 0);
        instr.push(1);
        instr.push(Opcode::LoadBool.byte());
        write_u16_le(&mut instr, 1);
        instr.push(0);
        instr.push(Opcode::BoolAnd.byte());
        write_u16_le(&mut instr, 2);
        write_u16_le(&mut instr, 0);
        write_u16_le(&mut instr, 1);
        instr.push(Opcode::Ret.byte());
        instr.push(0);
        let bytes = build_program(vec![encode_function("main", &[], &instr)]);
        run_exobyte(&bytes).expect("run");
    }
    ...
    fn vm_runs_quad_ops() {
        let mut instr = Vec::new();
        instr.push(Opcode::LoadQ.byte());
        write_u16_le(&mut instr, 0);
        instr.push(2);
        instr.push(Opcode::LoadQ.byte());
        write_u16_le(&mut instr, 1);
        instr.push(3);
        instr.push(Opcode::QAnd.byte());
        write_u16_le(&mut instr, 2);
        write_u16_le(&mut instr, 0);
        write_u16_le(&mut instr, 1);
        instr.push(Opcode::Ret.byte());
    ...
    fn vm_runs_call_ret() {
        let mut one = Vec::new();
        one.push(Opcode::LoadI32.byte());
        write_u16_le(&mut one, 0);
        write_i32_le(&mut one, 1);
        one.push(Opcode::Ret.byte());
        one.push(1);
        write_u16_le(&mut one, 0);

        let mut main = Vec::new();
        main.push(Opcode::Call.byte());
        main.push(1);
        write_u16_le(&mut main, 0);
        write_u16_le(&mut main, 0);
        write_u16_le(&mut main, 0);
        main.push(Opcode::Ret.byte());
        main.push(0);

        let bytes = build_program(vec![
            encode_function("main", &["one"], &main),
            encode_function("one", &[], &one),
        ]);
        run_exobyte(&bytes).expect("run");
    }
    ...
    fn vm_runs_f64_math_and_builtins() {
        let mut instr = Vec::new();
        instr.push(Opcode::LoadF64.byte());
        write_u16_le(&mut instr, 0);
        write_f64_le(&mut instr, 9.0);
        instr.push(Opcode::LoadF64.byte());
        write_u16_le(&mut instr, 2);
        write_f64_le(&mut instr, 1.0);
        instr.push(Opcode::LoadF64.byte());
        write_u16_le(&mut instr, 3);
        write_f64_le(&mut instr, 2.0);
        instr.push(Opcode::AddF64.byte());
        write_u16_le(&mut instr, 4);
        write_u16_le(&mut instr, 2);
        write_u16_le(&mut instr, 3);
        instr.push(Opcode::Call.byte());
        instr.push(1);
        write_u16_le(&mut instr, 1);
        write_u16_le(&mut instr, 0);
        write_u16_le(&mut instr, 1);
        ...
        let bytes = build_program(vec![encode_function("main", &["sqrt"], &instr)]);
    ...
    fn vm_reports_stack_overflow_on_unbounded_recursion() {
        ...
        recur.push(Opcode::Call.byte());
        recur.push(0);
        write_u16_le(&mut recur, 0);
        write_u16_le(&mut recur, 0);
        write_u16_le(&mut recur, 0);
        recur.push(Opcode::Ret.byte());
        recur.push(0);
        ...
        let bytes = build_program(vec![
            encode_function("main", &["recur"], &main),
            encode_function("recur", &["recur"], &recur),
        ]);
